...
export default function PredictionCard({ stats, recentGames = [] }) {
  ...
  const [tab, setTab] = useState('splits') // 'splits' | 'games'
  ...
  const formLabel = wins>=4?'Hot 🔥':wins>=3?'Good form':wins>=2?'Mixed':'Cold ❄️'
  const formColor = wins>=4?'#34d399':wins>=3?'#4f8cff':wins>=2?'#f59e0b':'#f87171'

  // Mini sparkline from last 5 pts
