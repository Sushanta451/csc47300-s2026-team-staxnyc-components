/**
 * Team display colors + ESPN CDN logos (no logo URL in Supabase player_stats).
 * Extend ABBR_BY_TEAM_NAME if your DB uses different `team` strings.
 */
...
/** Uppercase tricode (e.g. LAL) -> full team name, for standings rows that store abbreviations only. */
...
/**
 * Resolve URL slug + standings to canonical franchise name and BR-style tricode (e.g. lal).
 * `standingsRows[].team` may be a full name or a tricode depending on your `nba_standings` / static data.
 * @returns {{ canonicalName: string, tricode: string | null } | null}
 */
...
/** Distinct `team` column values to match in Supabase `.in()` */
...
/** @returns {{ abbr: string, color: string, logoUrl: string } | null} */
